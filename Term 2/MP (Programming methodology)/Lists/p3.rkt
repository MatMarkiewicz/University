...
;;; Data structure that represents signed messages
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ((encrypted-message (RSA-encrypt message recipient-public-key)))
    (make-signed-message (RSA-transform (compress encrypted-message) sender-private-key) encrypted-message)))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ((sign (signature cyphertext))
        (mess (message cyphertext)))
    (if (= (RSA-transform sign sender-public-key) (compress mess))
        (RSA-decrypt mess recipient-private-key)
        (error "Bad signature!"))))
...
(define (test message)
  (let ((user1 (generate-RSA-key-pair))
        (user2 (generate-RSA-key-pair)))
  (authenticate-and-decrypt (encrypt-and-sign message
                  (key-pair-public user2)
                  (key-pair-private user1))
                            (key-pair-private user2)
                            (key-pair-public user1))))
