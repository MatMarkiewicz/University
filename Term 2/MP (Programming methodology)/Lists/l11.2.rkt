...
(define (node-subtrees x)
  (third x))

(: make-node (All (A B) (-> A (Listof B) (Node A B))))
(define (make-node v ls)
  (list 'node v ls))

(: values (All (A) (-> (Tree A) (Listof A))))
(define (values t)
  (if (leaf? t)
      null
      (cons (node-val t) (append-map (lambda ([x : (Tree A)]) (values x)) (node-subtrees t)))))

(define test-tree (make-node 5
                             (list (make-node 4
                                              (list (make-node 3
                                                               (list leaf leaf))
                                                    leaf))
...
(define-type BinopAppend (U 'append))
...
; (eval (empty-env) (expr-binop `append "abc" "def"))
; (typeinfer (empty-env) (expr-binop `append "abc" "def"))