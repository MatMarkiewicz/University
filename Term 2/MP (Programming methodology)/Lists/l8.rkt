...
#|
(eval `((lambda-rec (append l1 l2) (if (null? l1) l2 (cons (car l1) (append (cdr l1) l2))))
        (cons (+ 1 1) null)
        (cons (+ 2 2) (cons (+ 4 4) null))
        ))

(eval `((lambda-rec (map f l1) (if (null? l1) null (cons (f (car l1)) (map f (cdr l1)))))
        (lambda (e) (* e e))
        (cons 1 (cons 2 null))))

(eval `((lambda-rec (reverse xs) (let (append (lambda-rec (append l1 l2) (if (null? l1) l2 (cons (car l1) (append (cdr l1) l2)))))
          (let (reverse (lambda-rec (reverse l1) (if (null? l1) null (append (reverse (cdr l1)) (cons (car l1) null)))))
            (reverse xs)))) (cons 1 (cons 2 (cons 3 null)))))
...
(eval `(let (lcons (lambda (x f) (cons x f)))
         (let (lhead (lambda (e) (car e)))
           (let (ltail (lambda (e) ((cdr e))))
             (let (nats-from (lambda-rec (nats-from m)
                                         (lcons
                                          m
                                          (lambda () (nats-from (+ m 1))))))
               (let (nats (nats-from 0))
                 (let (take (lambda-rec (take n l) (cond ((= n 0) null)
                                                         ((null? l) null)
                                                         (true (cons (lhead l) (take (- n 1) (ltail l)))))))
                   (let (testlist-from (lambda-rec (testlist-from m)
                                                   (lcons
                                                    m
                                                    (if (< m 5)
                                                        (lambda () (testlist-from (+ m 1)))
                                                        (lambda () null)))))
                     (let (testlist (testlist-from 0))
                       (let (append (lambda-rec (append xs ys) (if (null? xs)
                                                                   ys
                                                                   (lcons (lhead xs)
                                                                          (lambda () (append (ltail xs) ys))))))
                         (take 10 (append testlist testlist))
                   ))))))))))
|#

;(eval `((lambda x (+ (car (cdr x)) (car (cdr (cdr x))))) 1 2 3 4 5 6))
