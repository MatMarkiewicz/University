    ...
    for {
		...
		if !ENABLE_MAST || explore < EPSILON{
			...
			for i:=0;i<valid_action_count;i++{
				...
				if float64(N_MAST[actual_player-1][action.x][action.y])/W_MAST[actual_player-1][action.x][action.y] > maxQ{
					action_index = i
					maxQ = float64(N_MAST[actual_player-1][action.x][action.y])/W_MAST[actual_player-1][action.x][action.y]
				}
			}
		}
...
func backprop(v *MCTS_tree, final_winner int, actions_done []pair, noad int){
	...
	for i:=0;i<noad;i++{
		...
		N_MAST[last_player-1][act_action.x][act_action.y]++
		if last_player==final_winner{
			W_MAST[last_player-1][act_action.x][act_action.y] = W_MAST[last_player-1][act_action.x][act_action.y] + win_reward
		} else if final_winner==last_player{
			W_MAST[last_player-1][act_action.x][act_action.y] = W_MAST[last_player-1][act_action.x][act_action.y] + lose_reward
		} else {
			W_MAST[last_player-1][act_action.x][act_action.y] = W_MAST[last_player-1][act_action.x][act_action.y] + draw_reward
		}
	}
	last_player = v.last_player
	for {
